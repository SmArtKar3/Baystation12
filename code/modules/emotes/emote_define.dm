	...
	if(ismob(user))
		...
		//[INF]
			if(isliving(user))
				...
				if(L.silent)
					L.visible_message(message = "[user] noiselessly opens mouth!", self_message = "You cannot say anything!", blind_message = emote_message_impaired, checkghosts = /datum/client_preference/ghost_sight)
				else
		//[/INF]			
